...
```mermaid
graph LR
    User[ユーザー] -->|画像を送る| Server["サーバー (Go API)"]
    Server -->|加工された画像| User
```
...
```mermaid
flowchart TD
    subgraph script["負荷テストスクリプト"]
        IMG["sample.jpg"] --> R1["リサイズ"]
        IMG --> R2["グレースケール"]
        IMG --> R3["回転"]
        IMG --> RN["... 合計100リクエスト"]
    end
    R1 & R2 & R3 & RN -->|"10個ずつ同時に送信"| SRV["サーバー"]
    SRV --> RESULT["100人のユーザーが同時アクセスした状況を再現"]
```
...
```mermaid
flowchart TD
    A["スクリプトが sample.jpg を使って\n100回リクエストを送る"] --> B["サーバーが画像処理でCPUを使う"]
    B --> C["CPU使用率が50%を超える"]
    C --> D["HPAが検知して\n「Podを増やせ」と指示"]
    D --> E["新しいPodが起動する"]
    E --> F["負荷が分散される"]
